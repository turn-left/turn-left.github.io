...
3. `GenericApplicationContext`：继承了AbstractApplicationContext，实现了`BeanDefinitionRegistry`接口，拥有了所有`ApplicationContext`的功能，并且可以注册`BeanDefinition`，注意这个类中有一个属性(`DefaultListableBeanFactory` **beanFactory**)
4. AnnotationConfigRegistry：可以单独注册某个为类为BeanDefinition（可以处理该类上的`@Configuration`注解，已经可以处理**@Bean注解**），同时可以扫描
...
Spring中提供的类型转化服务，它比`PropertyEditor`更强大
...
如何向Spring中注册`ConversionService`：
...
整合了`PropertyEditor`和`ConversionService`的功能，是Spring内部用的
...
另外，Spring中还提供了一个`OrderComparator`的子类：`AnnotationAwareOrderComparator`，它支持用`@Order`来指定order值。比如：
...
`BeanPostProcessor`表示Bean的后置处理器，我们可以定义一个或多个`BeanPostProcessor`，比如通过一下代码定义一个`BeanPostProcessor`：
...
一个`BeanPostProcessor`可以在**任意一个Bean**的**初始化之前**以及**初始化之后**去额外的做一些用户自定义的逻辑，当然，我们可以通过判断beanName来进行针对性处理（针对某个Bean，或某部分Bean）。

我们可以通过定义`BeanPostProcessor`来干涉Spring创建Bean的过程。

### BeanFactoryPostProcessor

BeanFactoryPostProcessor表示Bean工厂的后置处理器，其实和`BeanPostProcessor`类似，`BeanPostProcessor`是干涉Bean的创建过程，`BeanFactoryPostProcessor`是干涉`BeanFactory`的创建过程。比如，我们可以这样定义一个`BeanFactoryPostProcessor`：
...
需要注意的是，`SimpleMetadataReader`去解析类时，使用的**ASM技术**。
